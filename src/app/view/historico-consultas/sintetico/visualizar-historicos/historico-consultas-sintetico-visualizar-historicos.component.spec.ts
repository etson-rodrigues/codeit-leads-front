...
describe('HistoricoConsultasSinteticoVisualizarHistoricosComponent', () => {
  ...
  beforeEach(
    waitForAsync(() => {
        ...
        const historicoConsultasServiceSpy = jasmine.createSpyObj('HistoricoConsultasService', ['getSinteticoBy', 'getSaldoDisponivel']);
        const exportarHistoricoConsultasServiceSpy = jasmine.createSpyObj('ExportarHistoricoConsultasService', ['export']);
  ...
  it('[CIT-5944] deve validar o campo tipo consulta', () => {
    ...
    tipoConsultaInput?.setValue(TipoConsulta.Exportacao);
    expect(tipoConsultaInput!.value).withContext('Campo tipo consulta deve ser "Exportacao"').toBe('Exportacao');
  });
  ...
  it('[CIT-5944] deve realizar exportação dos historicos pesquisados', () => {
    ...
    exportarHistoricoConsultasService.export.and.returnValue(of(fakeResponse));
    spyOn(component.downloadAnchor, 'click');
    component.export();
    expect(exportarHistoricoConsultasService.export).withContext('Deve chamar serviço de exportação após confirmação do usuário').toHaveBeenCalled();
  });

  it('[CIT-5944] deve retornar erro caso exportação dos historicos pesquisados falhar', () => {
    spyOn(component['_dialog'], 'open').and.returnValue({ afterClosed: () => of(true) } as MatDialogRef<typeof component>);
    exportarHistoricoConsultasService.export.and.returnValue(throwError(() => new Error()));
    component.export();
    expect(exportarHistoricoConsultasService.export).withContext('Deve chamar serviço de exportação após confirmação do usuário').toHaveBeenCalled();
